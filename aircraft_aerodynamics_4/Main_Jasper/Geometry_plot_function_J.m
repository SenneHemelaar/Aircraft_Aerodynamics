function [X_wing, Y_wing, Z_wing, X_winglet, Y_winglet, Z_winglet]...
         = Geometry_plot_function_J(lw,  Lam,  phiw,  cwr,  lam,  epsR,  epsT,...
...
winglet_geom = [       cR-cwr      ,       L      ,     0.0    ,cwr,-epsR ;
                cR-cwr+lw*sin(Lam),L+lw*sin(phiw)*cos(Lam),lw*cos(phiw)*cos(Lam),cwt,-epsT];
...
LE_X_wing = cosspace_J(wing_geom(1,1),wing_geom(2,1),NS1);
LE_Y_wing = cosspace_J(wing_geom(1,2),wing_geom(2,2),NS1);
...
if winglet_geom(2,1) > winglet_geom(1,1)
    LE_X_winglet = cosspace_J(winglet_geom(1,1),...
        winglet_geom(2,1),NS2);
elseif winglet_geom(1,1) > winglet_geom(2,1)
    LE_X_winglet = fliplr(cosspace_J(winglet_geom(2,1),...
...
if winglet_geom(2,2) > winglet_geom(1,2)
    LE_Y_winglet = cosspace_J(winglet_geom(1,2),winglet_geom(2,2),NS2);
elseif winglet_geom(2,2) < winglet_geom(1,2)
    LE_Y_winglet = fliplr(cosspace_J(winglet_geom(2,2),...
...
if winglet_geom(2,3) > winglet_geom(1,3)
    LE_Z_winglet = cosspace_J(winglet_geom(1,3),winglet_geom(2,3),NS2);
elseif winglet_geom(2,3) < winglet_geom(1,3)
    LE_Z_winglet = fliplr(cosspace_J(winglet_geom(2,3),...
...
for i = 1:NS1
    tempX = cosspace_J(LE_X_wing(i),wing_geom(1,4),NC1);
    tempY = LE_Y_wing(i);
    for j = 1:NC1
        X_wing(j,i) = tempX(j);
        Y_wing(j,i) = tempY;
...
for i = 1:NS2
    tempX = cosspace_J(LE_X_winglet(i),TE_X_winglet(i),NC2);
